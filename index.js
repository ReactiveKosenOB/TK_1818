...
function insertEvalDataToDBCallBack(postID, type, pushUserID) {
    ...
    MongoClient.connect(mongodbURI, (error, client) => {    
        var collection;
        const db = client.db(mongodbAddress);     // コレクションの取得
            
        collection = db.collection('eval');
        collection.insertOne(ret_evalData, (error, result) => {         console.log("inserted!");     });
    });
...
function showMyPost(event, userData) {
    getDBData(event, 'post', { userID: userData.userID, date: { '$ne': "pending" } }, function(event, condition, find) {
        ...
        //LINEMessageに配列を連想配列にして入れるとカルーセルもらえる
        console.log(JSON.stringify(conts))
        var msg = new LINEMessage({ 'content': conts }).makeCarousel(conts).makeFlex('投稿内容表示')
        if (conts.length != 0) {
            sendQuery(event.replyToken, msg)
        }
    });
...
function getUserDataFromDB(event, userID, callback) {
    MongoClient.connect(mongodbURI, (error, client) => {    
        ...
        const db = client.db(mongodbAddress);      // コレクションの取得
            
        collection = db.collection('users');      // コレクション中で条件に合致するドキュメントを取得
            
...
function updateUserData(userData) {
    MongoClient.connect(mongodbURI, (error, client) => {    
        ...
        collection.updateMany({ userID: userData.userID }, { $set: userData },
            (error, result) => {
                console.log("updated!");
            });
    });
...
function makeNewUserData(userID) {
    ...
    MongoClient.connect(mongodbURI, (error, client) => {    
        ...
            
        collection = db.collection('users');
        collection.insertOne(ret_userData, (error, result) => {         console.log("inserted!");     });
    });
    return ret_userData;
}
    ...
    Morphological.magnitude(text, function(sentiment_data, ret2) {
        ...
        Morphological.morphological(ret2, function(ret3) {
            ...
            MongoClient.connect(mongodbURI, (error, client) => {    
                ...
                collection.insertOne(ret_postData, (error, result) => {         console.log("inserted!");     });
            });
        })
    })
}
...
function fixPostData(userID) {
    MongoClient.connect(mongodbURI, (error, client) => {    
        ...
            
        collection = db.collection('post');
        collection.updateMany({ userID: userID, date: "pending" }, { $set: { date: getNowDateString() } },
            (error, result) => {
                console.log("fixed!!");
            });
    });
...
function evalPostData(postID, type) {
    MongoClient.connect(mongodbURI, (error, client) => {    
        ...
            
        collection = db.collection('post');
        collection.updateMany({ postID: postID }, { $inc: sp },
            (error, result) => {
                console.log("fixed!!");
            });
    });
...
function deletePendingPostData(userID) {
    MongoClient.connect(mongodbURI, (error, client) => {    
        ...
            
        collection = db.collection('post');
        collection.deleteOne({ userID: userID, date: "pending" }, (error, result) => {         console.log("deleted!");     });
    });
}
